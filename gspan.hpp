...
namespace graph_alg
{
	...
	// -----------------------------------------------------
	template<class GraphOps>
	class EdgeIterator<GraphOps, boost::undirected_tag>
	{
	public:
	...
	template<class GraphOps>
	EdgeIterator<GraphOps,boost::undirected_tag>::EdgeIterator(typename Traits<GraphOps>::VI vi,
								   const Graph& graph,
								   const GraphOps& ops)
	...
	template<class GraphOps>
	void EdgeIterator<GraphOps,boost::undirected_tag>::increment()
	{
	    if (++ou_iters_.first != ou_iters_.second)
		set_edge_ou();
	}
		   ...
		   const GraphOps& ops, Output& result)
	{
		...
		detail::one_edges(root, *tg_begin, ops);
...
#endif
			dfsc.push(ec);
			detail::project(i3->second, dfsc, minsup, ops, result);
			dfsc.pop();
		    }
	}
    }
